...
public class Konvertering {
    private static Konvertering konvertering = new Konvertering();

    private Konvertering() {

    }

    public static UtmPosisjon latLonToUtm(Location location) {
        return location== null ? null : latLonToUtm(location.getLatitude(), location.getLongitude());
    }

    public static UtmPosisjon latLonToUtm(double latitude, double longitude) {
        return konvertering.latLon2UTM(latitude, longitude);
    }

    public static class UtmPosisjon
    {
        private int utmNord;
        private int utmØst;

        public UtmPosisjon(String sonebelte, int utmØst , int utmNord) {
            this.utmNord = utmNord;
            this.utmØst = utmØst;
            this.sonebelte = sonebelte;
        }

        @Override
        public String toString() {
            return sonebelte + " " + utmØst + " " + utmNord;
        }

        public int getUtmNord() {
            return utmNord;
        }

        public int getUtmØst() {
            return utmØst;
        }

        private String sonebelte;


    }


    private UtmPosisjon latLon2UTM(double latitude, double longitude) {
    ...
    private class LatLon2UTM {
        public UtmPosisjon convertLatLonToUTM(double latitude, double longitude) {
            ...
            UtmPosisjon utmPosisjon = new UtmPosisjon(longZone + latZone, (int) _easting, (int) _northing);
